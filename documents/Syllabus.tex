...
Download the file {\small\texttt{UnxUpdates.zip}}, not
{\small\texttt{UnxUtils.zip}}.
After downloading
...
Tip: if sort2 refuses to work with a reference to some file
{\small\texttt{/tmp/<gibberish>}}, type
{\small\texttt{mkdir~\textbackslash{}tmp}}; this creates a temporary directory
that is used by many *nix commands as scratch space.
...
\noindent{\small\texttt{
ruby -e \doublequote\$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install)\doublequote}
...
-p data/db}} to complete the install. Each time you{\textquoteright}d
like to use MongoDB, open the terminal, go to the right folder, and run
{\small\texttt{bin/mongod -{}-dbpath data/db}}.
...
\~{}/data/db}}. Running the command {\small\texttt{bin/mongod -{}-dbpath
...
called {\small\texttt{pip-3.5}}, {\small\texttt{pip-3.6}} or {\small\texttt{pip3}}; replace the following commands with
the name that works on your system). To install the Spyder editor,
type the following two commands into the Terminal: 

\noindent{\small\texttt{pip3 install PyQt5}}
\\
{\small\texttt{pip3 install spyder}}

As often as not, the program spyder is called spyder3 after these installation
steps. Check what works for you.
