...
then paste them in a new folder \textit{C:{\textbackslash}Program~Files{\textbackslash}unxutils}.
...
winkey, type \texttt\small{cmd} and hit return) and type
...
command \texttt\small{path} to see if the path is correct.

----------> the section on cmd utils documentation
...
\textit{WinPython Command Prompt.exe} and use \texttt\small{pip install}.
...
the setup process. Then create the folder
...
and create a subfolder \texttt\small{data/db} with the command \texttt\small{mkdir
...
\texttt\small{bin/mongod -{}-dpath data/db}.

Alternatively, MongoDB can be installed with the command \texttt\small{brew
install mongodb -{}-with-openssl. }Subsequently, create a mongodb data
directory in your home directory with the command \texttt\small{mkdir -p
\~{}/data/dp }. Running the command \texttt\small{bin/mongod -{}-dpath
\~{}/data/db} should show you the following message.
\includegraphics[width=6.4953in,height=0.1783in]{Syllabus-img6.png} 

\subsubsection*{Spyder editor}
Python comes with a package manager called \texttt\small{pip} (sometimes also
called \texttt\small{pip-3.5 or pip-3.6; replace the following commands with
the name that works on your system).} To install the Spyder editor,
type the following two commands into the Terminal: 

\texttt\small{pip install PyQt5}
\\
\texttt\small{pip install spyder}

\subsubsection*{Python packages}
In a similar way, use \texttt\small{pip install
